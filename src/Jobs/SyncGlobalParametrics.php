...
/**
 * Job to synchronize global parametrics for a specific account.
 *
 * This job fetches and stores global parametrics like payment methods, document types, etc.
 * It uses exponential backoff for retries and prevents overlapping executions.
 */
class SyncGlobalParametrics implements ShouldQueue
{
    ...
    {
        return [(new WithoutOverlapping("sync-global-parametrics-".$this->account->id))->releaseAfter(10)];
    }

    /**
     * Execute the job.
     * Synchronizes global parametrics for the account.
     *
    ...
    {
        try {
            info("=====> Starting synchronization of GLOBAL parametrics for account {$this->account->id}");

            // List of global parametric types to sync
            $types = [
                ParametricType::MOTIVO_ANULACION,
                ParametricType::TIPOS_DOCUMENTO_IDENTIDAD,
                ParametricType::METODOS_DE_PAGO,
                ParametricType::UNIDADES
            ];
            ...
            info("=====> Finished synchronization of GLOBAL parametrics for account {$this->account->id}");
        } catch (\Throwable $ex) {
            info("Error syncing global parametrics for account {$this->account->id}: {$ex->getMessage()} in {$ex->getFile()}:{$ex->getLine()}");
    ...
    {
        info("Job SyncGlobalParametrics failed for account {$this->account->id} after {$this->tries} attempts: {$exception->getMessage()}");
